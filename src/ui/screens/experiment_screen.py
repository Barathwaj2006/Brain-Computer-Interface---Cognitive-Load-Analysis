...
class ExperimentScreen(QWidget):
    ...
    def init_ui(self):
        ...
        for i, (exp_title, exp_desc, exp_key) in enumerate(experiments):
            ...
            frame = QFrame()
            frame.setStyleSheet("background: rgba(15,23,42,0.8); border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; padding: 16px;")
            ...
            d = QLabel(exp_desc)
            d.setStyleSheet("font-size: 11px; color: #94A3B8; margin: 6px 0;")
            d.setWordWrap(True)

            btn = QPushButton("▶ LAUNCH EXPERIMENT")
            btn.setStyleSheet("background: linear-gradient(135deg, #06B6D4, #0284C7); color: white; font-weight: 800; font-size: 11px; padding: 10px; border-radius: 6px; border: none;")
